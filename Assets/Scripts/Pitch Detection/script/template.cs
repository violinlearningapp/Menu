...
public class template : MonoBehaviour
{
    ...
    public Text noteUS;							//UI txt where the note will be printed
    public Text noteEU;
    public Text freqNote;
    public Text noteMIDI;


    // public GameObject noteSprite;
    //public GameObject sharpNoteSprite;
    ...
    void Update()
    {
        ...
        if (dbValue < minVolumeDB)
        {
            ...
            //sharpNoteSprite.SetActive(false);
        ...
        int midi = findMode();
        noteUS.text = "Note US: " + pitchDetector.midiNoteToString(midi);
        noteEU.text = "Note EU: " + pitchDetector.midiNoteToStringEU(midi);
        freqNote.text = "Freq: " + pitchDetector.lastFrequency();
        noteMIDI.text = "Midi: " + midiant;
        /*
        string noteDisplay = pitchDetector.midiNoteToStringClean(midi);
        ...
        switch (noteDisplay)
        {
            case "Do":
                ...
                sharpNoteSprite.SetActive(false);
                noteHeight = -0.67;
                noteSprite.transform.position = new Vector2((float)7.7, (float)noteHeight);
                break;
            case "Ré":
                noteSprite.SetActive(true);
                sharpNoteSprite.SetActive(false);
            ...
            case "Do #":
                noteSprite.SetActive(false);
                sharpNoteSprite.SetActive(true);
                noteHeight = -0.67;
                sharpNoteSprite.transform.position = new Vector2((float)7.7, (float)noteHeight);
                break;
            case "Ré #":
                noteSprite.SetActive(false);
                sharpNoteSprite.SetActive(true);
